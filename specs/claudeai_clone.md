...
Build a fully functional clone of claude.ai, Anthropic's conversational AI interface. The application provides a clean, modern chat interface for interacting with Claude via the API, including features like conversation management, artifact rendering, project organization, multiple model selection, and advanced settings. The UI closely matches claude.ai's design using Tailwind CSS with a focus on excellent user experience and responsive design.
...
- **Configuration**: JSON-driven config via `config/claudeai.json` (no implicit .env loading)
...
- **API Integration**: Claude API for chat completions
...
All configuration stored in `config/claudeai.json`:

- Server ports (frontend: 3330, backend: 3331)
- Database path: `file:../../data/claudeai.db`
...
- Application configuration stored in `config/claudeai.json`
...
### Step 1: Project Foundation and Database
...
- Create config/claudeai.json and environment-specific variants
...
- Virtual scrolling for large lists (10,000+ conversations)
